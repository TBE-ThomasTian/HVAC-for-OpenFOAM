...
{
    const label rounded = label(std::round(normalizeDirection(windFrom)));
    return prefix + "Gamma_" + Foam::name(rounded == 360 ? 0 : rounded);
}
...
)
{
    ...
    return Foam::exp(-Foam::pow(referenceThreshold/scale, shape));
}
...
)
{
    ...
    forAll(entries, directionI)
    {
        ...
        spec.windFrom = normalizeDirection(entry.get<scalar>("windFrom"));
        ...
        if (spec.referenceHeight <= VSMALL)
        {
            FatalIOErrorInFunction(entry)
...
)
{
    ...
    if (referenceHeight <= VSMALL || angleTolerance < 0)
    {
        FatalIOErrorInFunction(climateDict)
            << "referenceHeight must be positive and directionTolerance "
    ...
    forAll(entries, sectorI)
    {
        ...
        sector.windFrom = normalizeDirection(entry.get<scalar>("windFrom"));
    ...
    if (totalFrequency > 1.0 + 1e-8)
    {
        FatalIOErrorInFunction(climateDict)
            << "Climate frequencies sum to " << totalFrequency
            << ", but joint annual fractions may not sum above one"
            << exit(FatalIOError);
    }

    if (totalFrequency < 1.0 - 1e-8)
    {
        Info<< "Climate sector frequencies sum to " << totalFrequency
            << "; treating the remaining " << 1.0 - totalFrequency
            << " as calm/non-exceeding time." << nl;
    }
...
(
    const vectorField& referenceCentres,
    const fvMesh& mesh,
    const scalar tolerance,
    const fileName& caseDir
)
{
    if (mesh.nCells() != referenceCentres.size())
    {
        FatalErrorInFunction
            << "Cell count differs between the reference mesh ("
            << referenceCentres.size() << ") and " << caseDir << " ("
            << mesh.nCells() << "). Use matching meshes and coordinates."
            << exit(FatalError);
    }

    const vectorField& centres = mesh.cellCentres();
    scalar maxCentreDifference = 0;
    forAll(referenceCentres, cellI)
    {
        maxCentreDifference = max
        (
            maxCentreDifference,
            mag(referenceCentres[cellI] - centres[cellI])
        );
    }
...
int main(int argc, char *argv[])
{
    ...
    const bool strictDirectionCount =
        nenDict.getOrDefault<bool>("strictDirectionCount", false);
    ...
    if (mag(vertical) <= VSMALL || pedestrianHeight < 0)
    {
        FatalIOErrorInFunction(surfaceDict)
    ...
    {
        FatalIOErrorInFunction(nenDict)
            << "geometryTolerance must be non-negative"
            << exit(FatalIOError);
    }
    ...
    vectorField referenceCellCentres;
    ...
    forAll(directions, directionI)
    {
        ...
        if (directionI == 0)
        {
            referenceCellCentres = mesh.cellCentres();
            if (hasClimate)
            {
    ...
    scalarField p5Percent(referenceCellCentres.size(), 0);
    scalarField p15Percent(referenceCellCentres.size(), 0);
    scalarField comfort(referenceCellCentres.size(), 0);
    scalarField traversing(referenceCellCentres.size(), 0);
    scalarField strolling(referenceCellCentres.size(), 0);
    scalarField sitting(referenceCellCentres.size(), 0);
    scalarField danger(referenceCellCentres.size(), 0);

    if (hasClimate)
    {
        forAll(referenceCellCentres, cellI)
        {
